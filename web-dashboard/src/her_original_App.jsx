...
// ΓöÇΓöÇΓöÇ Constants ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
const NAV_ITEMS = [
  { id: 'dashboard',  label: 'Dashboard',  icon: 'Γ¼¢' },
  { id: 'analytics',  label: 'Analytics',  icon: 'Γùê'  },
  { id: 'emissions',  label: 'Emissions',  icon: '≡ƒî┐' },
  { id: 'rankings',   label: 'Rankings',   icon: '≡ƒÅå' },
];

// ΓöÇΓöÇΓöÇ Score Ring ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇΓöÇ KPI Card ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇΓöÇ Alert Banner ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
function AlertBanner({ routes, onDismiss }) {
  ...
  return (
    <div className="alert-banner" role="alert">
      ...
      <span>
        <strong>ΓÜá HIGH CONGESTION ALERT</strong> ΓÇö {highRoutes.map(([id]) => id.toUpperCase()).join(', ')} detected
        with critical congestion levels. Immediate rerouting recommended.
      </span>
      <button className="alert-close" onClick={onDismiss} aria-label="Dismiss alert">Γ£ò</button>
    </div>
  );
}

// ΓöÇΓöÇΓöÇ Route Detail Modal ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
function RouteModal({ routeId, congestion, history, onClose }) {
  ...
  const stats = [
    { label: 'Speed',          value: hist.speed            ? `${hist.speed.toFixed(2)} m/s`           : 'N/A', icon: 'ΓÜí' },
    { label: 'COΓéé Emission',   value: hist.co2_emission     ? `${hist.co2_emission.toFixed(1)} mg`      : 'N/A', icon: '≡ƒÆ¿' },
    { label: 'CO Emission',    value: hist.co_emission      ? `${hist.co_emission.toFixed(2)} mg`       : 'N/A', icon: '≡ƒî½' },
    { label: 'NOx Emission',   value: hist.nox_emission     ? `${hist.nox_emission.toFixed(3)} mg`      : 'N/A', icon: 'ΓÜù' },
    { label: 'Fuel Consumption', value: hist.fuel_consumption ? `${hist.fuel_consumption.toFixed(3)} ml/s` : 'N/A', icon: 'Γ¢╜' },
    { label: 'FAHP Score',     value: data.congestion_score?.toFixed(2),                                         icon: '≡ƒôè' },
    { label: 'Hybrid Score',   value: data.hybrid_score?.toFixed(2) || 'N/A',                                    icon: '≡ƒºá' },
    { label: 'Algorithm Rank', value: `#${data.rank} of ${totalRoutes}`,                                         icon: '≡ƒÅå' },
    { label: 'Status',         value: `${data.status} CONGESTION`,                                               icon: '≡ƒÜª' },
  ];
    ...
    <div className="modal-overlay" onClick={onClose} role="dialog" aria-modal="true">
      <div className="modal-panel" onClick={e => e.stopPropagation()} style={{ '--modal-color': color }}>
        <div className="modal-header" style={{ borderColor: color + '40' }}>
          ...
          <button className="modal-close" onClick={onClose} aria-label="Close modal">Γ£ò</button>
        </div>
          ...
          {data.alternate_route && (
            <div style={{ background: 'rgba(46, 213, 115, 0.1)', border: '1px solid rgba(46, 213, 115, 0.3)', borderRadius: 12, padding: '12px 16px', marginBottom: 20 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, color: '#2ed573', fontWeight: 700, fontSize: 12, textTransform: 'uppercase', marginBottom: 4 }}>
                <span>Γ£¿ Intelligent Rerouting</span>
              </div>
...
// ΓöÇΓöÇΓöÇ Custom Recharts Tooltip ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇΓöÇ Main App ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
export default function App() {
  ...
  // ΓöÇΓöÇ Fetch all data ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  // ΓöÇΓöÇ Live clock ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  useEffect(() => {
    const t = setInterval(() => setClock(new Date()), 1000);
    return () => clearInterval(t);
  }, []);

  // ΓöÇΓöÇ Auto-refresh countdown ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  useEffect(() => {
    if (!autoRefresh) {
  ...
  // ΓöÇΓöÇ Pipeline refresh ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  // ΓöÇΓöÇ Sort handler ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  const handleSort = (key) => {
    setSortDir(prev => sortKey === key ? (prev === 'asc' ? 'desc' : 'asc') : 'asc');
    setSortKey(key);
  };

  // ΓöÇΓöÇ Loading / Error screens ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  if (loading) return (
  ...
  if (error) return (
    <div className="state-screen">
      <div className="error-icon">ΓÜá</div>
  ...
  // ΓöÇΓöÇ Derived data ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  // Radar chart data ΓÇö normalize each param to 0ΓÇô100
    ...
    <div className="app-shell">
      ...
      <aside className="sidebar" aria-label="Navigation sidebar">
        <div className="sidebar-brand">
          <div className="brand-icon">≡ƒÜª</div>
      ...
      <main className="main-content" role="main">
        ...
        <header className="topbar">
          ...
          <div className="topbar-actions">
            ...
            >
              Γƒ│ {autoRefresh ? `Auto ${countdown}s` : 'Auto'}
            </button>
            <button
            ...
            >
              <span className="btn-icon">Γƒ│</span> {refreshing ? 'Running...' : 'Re-run Algorithm'}
            </button>
          </div>
        </header>
        ...
        <div className="dashboard-wrap">
          ...
          <div className="kpi-row" aria-label="Key performance indicators">
            ...
            <KPICard
              ...
              value={summary?.worst_route?.id
                ...
                : 'ΓÇö'}
              subtitle={`Score: ${summary?.worst_route?.score?.toFixed(0) ?? 'ΓÇö'}`}
              color="#ff4757"
              pulse
            />
            <KPICard
              title="Avg Speed"
              value={summary?.avg_speed ? `${summary.avg_speed} m/s` : 'ΓÇö'}
              subtitle="Across all routes"
              color="#ffa502"
            />
            <KPICard
              title="Avg COΓéé"
              value={summary?.avg_co2 ? `${summary.avg_co2} mg` : 'ΓÇö'}
              subtitle="Mean emission level"
              color="#2ed573"
            />
          </div>

          {/* ΓöÇΓöÇ DASHBOARD TAB ΓöÇΓöÇ */}
          {activeTab === 'dashboard' && (
            <div className="tab-content">
                ...
                {routeEntries.map(([id, data]) => {
                  ...
                  return (
                    <div
                      ...
                      role="button"
                      tabIndex={0}
                      aria-label={`${id} ΓÇö ${data.status} congestion. Click for details.`}
                    ...
                    >
                      ...
                      <div className="card-hint">Click for full details ΓåÆ</div>
                    </div>
                  );
                })}
              </div>
              ...
              <div className="viz-row">
                ...
                <div className="glass-panel map-panel" aria-label="Live traffic map">
                  <div className="panel-title">≡ƒù║ Live Traffic Map ΓÇö Kalinga Hospital Junction</div>
                ...
                <div className="glass-panel" aria-label="Congestion score bar chart">
                  <div className="panel-title">≡ƒôè FAHP Congestion Scores</div>
            ...
            <div className="tab-content">
              <div className="analytics-grid">
                ...
                <div className="glass-panel" aria-label="Multi-parameter radar chart">
                  <div className="panel-title">≡ƒò╕ Multi-Parameter Radar Analysis</div>
                ...
                <div className="glass-panel" aria-label="Emissions comparison chart">
                  <div className="panel-title">≡ƒôë CO vs NOx Emissions by Route</div>
          ...
          {/* ΓöÇΓöÇ EMISSIONS TAB ΓöÇΓöÇ */}
          {activeTab === 'emissions' && (
            <div className="tab-content">
              {/* Summary Table */}
              <div className="glass-panel" style={{ marginBottom: 24 }} aria-label="Emissions data table">
                <div className="panel-title">≡ƒî┐ Emissions Summary Table ΓÇö All Routes</div>
                  ...
                  {Object.entries(history).map(([id, d]) => {
                    ...
                    return (
                      <div key={id} className="em-row">
                        ...
                        <span>{d.speed?.toFixed(2) ?? 'ΓÇö'}</span>
                        <span>{d.co_emission?.toFixed(2) ?? 'ΓÇö'}</span>
                        <span>{d.co2_emission?.toFixed(1) ?? 'ΓÇö'}</span>
                        <span>{d.nox_emission?.toFixed(3) ?? 'ΓÇö'}</span>
                        <span>{d.fuel_consumption?.toFixed(3) ?? 'ΓÇö'}</span>
                      </div>
                    );
                  })}
                </div>
              ...
              <div className="cards-grid-2" aria-label="Per-route emissions breakdown">
                {Object.entries(history).map(([id, d]) => {
                  ...
                  const bars = [
                    { label: 'COΓéé', value: d.co2_emission,    max: maxCO2,  color: '#ff6b81', unit: 'mg' },
                    ...
                    return (
                      ...
                      >
                        <span className="rank-medal">
                          {data.rank === 1 ? '≡ƒÑç' : data.rank === 2 ? '≡ƒÑê' : data.rank === 3 ? '≡ƒÑë' : `#${data.rank}`}
                        </span>
      ...
      {/* ΓöÇΓöÇ Route Detail Modal ΓöÇΓöÇ */}
      {selectedRoute && (
        <RouteModal
